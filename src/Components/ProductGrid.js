...
function ProductGrid() {
  ...
  const handleAddToCart = (product) => {
    if (cart && cart.addToCart) {
      cart.addToCart({ id: product.id, name: product.name, price: product.price, img: product.image });
    }
  };

  return (
    <section className={`product-grid ${filter === 'all' ? 'all-active' : ''}`}>
      <h2>Our Products</h2>
          ...
          <Link key={product.id} to={`/product/${product.id}`}>
            <article className="product">
              ...
              <div className="product-meta">
                ...
                <span className="rating" aria-label={`Rating ${product.rating}`}> {product.rating} </span>
              </div>
              <button className="btn" onClick={(e) => { e.preventDefault(); handleAddToCart(product); }}> Add to Cart </button>
            </article>
          </Link>
        ))}
      </div>
